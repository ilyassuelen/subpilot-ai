...
def serialize_attributes(attributes) -> list[dict]:
    """Convert Pydantic comparison attributes into plain dictionaries."""
    return [attribute.model_dump() for attribute in attributes or []]
...
def create_contract(db: Session, user_id: int, contract_data: ContractCreate) -> Contract:
    ...
    new_contract = Contract(
        ...
        comparison_attributes=serialize_attributes(contract_data.comparison_attributes),
    )
...
def update_contract(
    ...
    contract.plan_name = contract_data.plan_name
    ...
    contract.comparison_attributes = serialize_attributes(contract_data.comparison_attributes)
...
def get_expiring_contracts(db: Session, user_id: int, days: int = 30) -> list[Contract]:
    """Return contracts whose cancellation deadline is within the next given number of days."""
    today = date.today()
    target_date = today + timedelta(days=days)
    expiring_contracts = []

    for contract in get_all_contracts(db, user_id):
        cancellation_deadline = calculate_cancellation_deadline(contract)
